...
class MultiAgent:
    def __init__(self, deployment_name: str, maximum_iterations: int, selection_template: str, termination_template: str):
    ...
    def create_chat_group(self, agents: list):
        chat_group =  AgentGroupChat(
            ...
            selection_strategy=KernelFunctionSelectionStrategy(
                ...
                kernel=self._create_kernel_with_chat_completion("selection", self.deployment_name),
                result_parser=lambda result: str(result.value[0]) if result.value is not None else agents[-1].name,
                agent_variable_name="agents",
                history_variable_name="history",
            ),
            termination_strategy=KernelFunctionTerminationStrategy(
                function=self.termination_function,
                kernel=self._create_kernel_with_chat_completion("termination", self.deployment_name),
